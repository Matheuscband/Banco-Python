...
def menu() -> None:
    ...
    print('Selecione o serviço desejado no menu: ')
    print('1 - Criar conta')
...
def efetuar_saque() -> None:
    if len(contas) > 0:
        numero: int = int(input('Informe o número de sua conta: '))
        conta: Conta = buscar_conta_por_numero(numero)

        if conta:
            valor: int = int(input('Informe o valor do saque: '))

            conta.sacar(valor)
        else:
            print(f'Não foi encontrada a conta com número {numero}')
    else:
        print('Ainda não possui conta cadastrada.')
    sleep(2)
    menu()
    ...
    if len(contas) > 0:
        ...
        if conta_o:
            ...
            if conta_d:
                ...
                print(f'Esta conta {conta_d} não foi encontrada.')
        else:
